...
class DictSegment implements Comparable<DictSegment> {
  ...
  /// @param searchHit
  /// @return Hit
  Hit match(List<String> charArray, [int begin = 0, int? length, Hit? searchHit]) {
    ...
    if (segmentArray != null) {
      ...
      int position = binarySearch(segmentArray, keySegment, start: 0, end: storeSize);
      if (position >= 0) {
  ...
  void fillSegment(List<String> charArray, [int begin = 0, int? length, int enabled = 1]) {
    length ??= charArray.length;
    // 获取字典表中的汉字对象
  ...
  void migrate(List<DictSegment> segmentArray, Map<String, DictSegment> segmentMap) {
